...
st.markdown("""
    ...
    .timer-header {
        ...
        text-align: center; padding: 10px; border-bottom: 3px solid #1a73e8; z-index: 999;
    }
    .main { background-color: #fdfdfd; padding-top: 50px; }
    div[data-baseweb="select"] { border: 2px solid #1a73e8; border-radius: 4px; }
    </style>
...
if st.session_state.page == 'login':
    ...
    with st.container(border=True):
        ...
        app_num = st.text_input("Application Number (LAST 4-Digits Only)", max_chars=4)
        ...
        if st.button("Start TEST"):
            if not name or len(app_num) != 4 or not app_num.isdigit() or not dept:
                st.error("Invalid Entry: Ensure all fields are filled correctly.")
            elif has_submitted(name, app_num):
                st.warning("You have already completed this TEST.")
            else:
...
# --- EXAM VIEW ---
    ...
    remaining = max(0, 480 - int(elapsed)) 
    ...
    st.markdown(f'<div class="timer-header"><h2 style="color: #e53935; margin:0;">⏳ TIME REMAINING: {mins:02d}:{secs:02d}</h2></div>', unsafe_allow_html=True)
    st.write("---")
    st.header(f"Candidate: {st.session_state.student_name}")
    st.caption(f"{st.session_state.dept} | ID: {st.session_state.student_id}")

    for i, item in enumerate(st.session_state.questions):
        st.markdown(f"**Question {i+1}:** {item['q']}")
        st.session_state.responses[i] = st.selectbox("Choose Answer", item['o'], key=f"q_{i}", label_visibility="collapsed")
        st.divider()
...
# --- RESULT & ADMIN VIEW ---
    ...
    score = sum(1 for i, item in enumerate(st.session_state.questions) if st.session_state.responses.get(i) == item['a'])
    ...
    st.metric("Your SCORE is", f"{score} / 30")
    
    st.divider()
    
    # Real-time Score Board
    st.subheader("📊 Real-Time Performance Board")
    if os.path.exists(CSV_FILE):
        full_df = pd.read_csv(CSV_FILE)
        st.dataframe(full_df[["Full Name", "Department", "Score"]].sort_values("Score", ascending=False), 
                     use_container_width=True, hide_index=True)
        
        # Protected Data Download
        st.divider()
        st.subheader("📂 Lecturer Access Only")
        pw_input = st.text_input("Enter Admin Password to Download Data", type="password")
        if pw_input == ADMIN_PASSWORD:
            csv_data = full_df.to_csv(index=False).encode('utf-8')
            st.download_button(
                label="Download Full Results (CSV)",
                data=csv_data,
                file_name=f"GLT302_Final_Results.csv",
                mime='text/csv'
            )
        elif pw_input:
